...
public class SampleController {
    ...
    @GetMapping("/ex/ex1") // http://192.168.111.105:80/ex/ex1 -> void 니까 /resources/templates/ex/ex1.html 을 찾는다
    public void ex1(Model model){
        // 리스트 타입으로 데이터를 보내보자.
        List<String> list = Arrays.asList("최수빈","최연준","최범규","강태현","휴닝카이");
    ...
    public  void ex2(Model model){
        ...
        // 이너 클래스를 사용해서 객체를 뿌려보자. p426

        List<String> strList = IntStream.range(1,10) // 1~10까지의 정수를 생성한다.
                .mapToObj(i -> "데이터"+i)
        ...
        SampleDTO smapleDTO = new SampleDTO();
        smapleDTO.p1 = "값......p1";
        smapleDTO.p2 = "값......p2";
        smapleDTO.p3 = "값......p3";
        // 최종적으로 객체 3개 완성

        model.addAttribute("list", strList); // 79행에 만들어진 객체값
        model.addAttribute("map", map); // 84행에 만들어진 객체값
        model.addAttribute("dto", smapleDTO); // 88행에 만들어진 객체값
